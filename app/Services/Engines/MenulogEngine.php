...
/**
 * Menulog Engine (AU/NZ market)
 *
 * @package    ClaudeScraper
 * @subpackage Services\Engines
 * @author     J.J. Johnson <visionquest716@gmail.com>
 */
class MenulogEngine extends AbstractEngine
{
    public function getPlatformSlug(): string { return 'menulog'; }
    public function canHandle(string $url): bool { return (bool) preg_match('/menulog\.com\.au/i', $url); }
    ...
    {
        $items = [];
        if (preg_match('/"menuCategories"\s*:\s*(\[.*?\])\s*[,}]/s', $html, $m)) {
            $data = json_decode($m[1], true);
            if (is_array($data)) {
                foreach ($data as $cat) {
                    $catName = $cat['name'] ?? null;
                    foreach ($cat['products'] ?? $cat['items'] ?? [] as $mi) {
                        $price = isset($mi['price']) ? (float)$mi['price'] : null;
                        $items[] = $this->makeItem($mi['name'] ?? 'Unknown', $price, $mi['description'] ?? null, $catName, $mi['imageUrl'] ?? null);
                    }
                }
            }
        }
    ...
    {
        $items = [];
        foreach (['.menu-item','[class*="menu-item"]','[class*="MenuItem"]'] as $s) {
    ...
    {
        try{$this->http->head('https://www.menulog.com.au',['timeout'=>10]);return['healthy'=>true,'message'=>'Menulog reachable'];}catch(\Exception $e){return['healthy'=>false,'message'=>$e->getMessage()];}
    }
}
